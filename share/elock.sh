...
opt_usage elock <<'END'
elock is a wrapper around flock(1) to create a file-system level lockfile associated with a given filename. This is an
advisory lock only and requires all callers to use elock/eunlock in order to protect the file. This method is easier to
use than calling flock directly since it will automatically open a file descriptor to associate with the lockfile and
store that off in an associative array for later use.
...
Get the file descriptor (if any) that our process has associated with a given on-disk lockfile. This is largely for
convenience inside elock and eunlock to avoid some code duplication but could also be used externally if needed.
END
elock_get_fd()
{
    $(opt_parse fname)
...
Check if a file is locked via elock. This simply looks for the file inside our associative array because flock doesn't
provide a native way to check if we have a file locked or not.
END
elock_locked()
{
    $(opt_parse fname)
