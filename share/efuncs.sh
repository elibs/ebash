...
of a command in any way then bash effectively disables set -e that command invocation REGARDLESS OF DEPTH. "Handling the
failure" includes putting it in a while or until loop, part of an if/else statement or part of a command executed in a
&& or ||.
...
# Recursively unmount the named directories and remove them (if they exist) then create new ones. NOTE: Unlike earlier
# implementations, this handles multiple arguments properly.
efreshdir()
{
    local mnt
    for mnt in "${@}"; do
...
Returns true if the input string is an integer and false otherwise. May have a leading '-' or '+' to indicate the number
is negative or positive. This does NOT handle floating point numbers. For that you should instead use is_num.
END
is_int()
{
...
Returns true if the input string is a number and false otherwise. May have a leading '-' or '+' to indicate the number
is negative or positive. Unlike is_integer, this function properly handles floating point numbers.
